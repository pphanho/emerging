...
# 출원년도가 1990 ~ 2013년인 특허들만 선별
data.processed <- data.processed %>% filter(data.processed$출원년도 >= 1990) # 4316 obs. of 69 variables
data.processed <- data.processed %>% filter(data.processed$출원년도 <= 2013) # 3072 obs. of 69 variables
...
cpc_near_dic <- data.processed %>% filter(data.processed$출원년도 >= 2011 & data.processed$출원년도 <= 2013) #최근 3년 기준 
...
cpc_before_dic <- data.processed %>% filter(data.processed$출원년도 >= 2008 &data.processed$출원년도 <= 2010) #최근 3년 기준 
...
temp <- data.processed %>% filter(data.processed$출원년도 >= 2004 & data.processed$출원년도 <= 2008)
temp <- table(temp$출원인대표명) %>% sort(decreasing = T) %>% as.data.frame()
colnames(temp) <- c("corp","freq")

temp2 <- data.processed %>% filter(data.processed$출원년도 >= 2009 & data.processed$출원년도 <= 2013)
...
data.processed.selected <- data.processed.selected[, c(1, 10, 14:length(data.processed.selected))]
# data.processed.selected <- data.processed.selected[, c(1,2, 10, 14:length(data.processed.selected))] # 전문가DB 추가

# 종속변수 NA값 제거 (NA의 이유 = 특허 자체가 사라진 특허임)
data.processed.selected <- data.processed.selected %>% filter(!is.na(data.processed.selected$피인용횟수)) # 3066 Records of 36 variables

# 전체 변수 NA값 여부 확인
naniar::miss_var_summary(data.processed.selected)

# 종속변수 설정 전까지의데이터 저장
# save.image(file = './data_processed_selected까지 진행(종속변수 설정 전).Rdata')
...
for(i in 1 : length(levels(temp_data$출원년도))){
  ...
  top_10_pct <- (nrow(year_data) * 0.10) %>% round(digits = 0) # 상위 10%에 해당하는 수를 변수 top_10_pct에 저장
...
# save.image(file = './citedby10%.Rdata') # 종속변수 피인용수 상위 10% 데이터 저장
...
# quali_data <- temp_data %>% purrr::keep(is.factor) # 질적 변수만 선별
# 
# stat.desc(quanti_data)
# stat.desc(quali_data)
# write.csv(quanti_data, './quanti_data.csv')
# write.csv(quali_data, './quali_data.csv')
...
colnames(temp_data)
skew_temp_data <- temp_data[, -c(10, 18:27, 35:40)] # 양적 변수만 선별
# skew_temp_data <- temp_data[, -c(1, 11, 19:28, 36)] # 전문가 db용 
...
colnames(temp_data)
zero_temp_data <- temp_data[, -c(10, 18:27, 35:40)] # 양적 변수만 선별
zero_temp_data <- temp_data[, -c(1, 11, 19:28, 36)] # 전문가 db용 

NearZeroValues <- nearZeroVar(zero_temp_data, saveMetrics = TRUE)

# 분산이 0인 변수 제거
temp_data <- temp_data[, -c(29, 30)]
# temp_data <- temp_data[, -c(30, 31)] # 전문가 db용

# C. Multicollinearity를 고려하여 변수간 상관계수 확인 및 상관계수가 너무 큰 변수 제거
colnames(temp_data)
cor_temp_data <- temp_data[, -c(10, 18:27, 33:38)] # 양적 변수만 선별
# cor_temp_data <- temp_data[, -c(1, 11, 19:28, 34)] # 전문가 db용
...
# save(temp_data, file = './temp_data(기본 전처리 완료).Rdata') # 전처리 완료된 최종 데이터 저장
# load(file = './temp_data(기본 전처리 완료).Rdata')

# save(temp_data, file = './temp_data(전문가DB활용).Rdata') # 전처리 완료된 최종 데이터 저장
