...
for(j in 1:loocv_var){
  ...
  md <- randomForest(formula = 핵심특허 ~ ., data = trdata, mtry = floor(sqrt(32)), ntree = 1000, importance = TRUE, keep.forest = TRUE)
  
  stopCluster(cl) # End parallel cluster
  
  # 지도학습만을 이용한 TrainSet 성능 평가
  true <- testdata$핵심특허
  pred <- predict(md, testdata, type = 'response')
  ...
  while(TRUE){
    ...
    # UnlabeledSet 예측
    temp_for_insert <- predict(md, unlabeledSet[1:unit,], type = 'vote') %>% as.data.frame()
...
# save.image(file = './expert_rf_ET.Rdata')
# load(file = './expert_rf_ET.Rdata')

save.image(file = './expert_rf_NET.Rdata')
# load(file = './expert_rf_NET.Rdata')
